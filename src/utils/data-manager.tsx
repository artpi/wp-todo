	...
	Promise.all( updatePromises ).then( ( responses ) => {
		...
		if ( data.taxonomy ) {
			const taxonomyUrl = new URL(
				data.taxonomies[ data.taxonomy ]._links[ 'wp:items' ][ 0 ].href
			);
			taxonomyUrl.searchParams.set( 'per_page', '100' ); // TODO change this to pull all
			authenticadedFetch(
				taxonomyUrl.toString(),
				{},
				login,
				pass,
				wpcomToken
...
function createDataManager(): DataManager {
		...
		if (
			...
			( ! remindersPermission ||
				( remindersPermission.status !== 'granted' &&
					remindersPermission.canAskAgain ) )
		) {
	...
	const loadStoredData = async () => {
		...
		if (
			url &&
			( storedWpcomToken || ( storedLogin && storedPass ) ) &&
			savedConfigObject.connected
		) {
			syncData(
				savedTodosObject,
				savedConfigObject,
	...
	const connectWP = useCallback(
		( wpcomToken: string, wpcomData ) => {
			...
			if ( wpcomToken.length > 0 && wpcomData ) {
				...
				siteData = fetch(
					normalizeUrl( url, 'https' ) + `?rest_route=/`
				)
					.catch( ( err ) =>
			...
			siteData.then( ( data ) => {
				console.log(
					'ROUTES',
					data.routes,
					getLinkToEndpoint( data.routes, '/users/me' )
				);
			} );
